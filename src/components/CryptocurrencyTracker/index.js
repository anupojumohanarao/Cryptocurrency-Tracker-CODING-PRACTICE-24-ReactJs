...
class CryptocurrencyTracker extends Component {
  ...
  componentDidMount() {
    this.getMainCryptoCurrency()
  }

  getMainCryptoCurrency = async () => {
    const response = await fetch(apiUrl)
    const totalData = await response.json()

    this.setState({
      cryptocurrenciesData: totalData.map(eachCryptocurrency => ({
  ...
  render() {
    ...
    return (
      <div className="main-crypto-container">
        {isLoading ? this.renderLoader() : this.renderMainCryptoList()}
      </div>
    )
  }
}
